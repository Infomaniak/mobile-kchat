 ...
 */
export const transformTeamMembershipRecord = ({
    action,
    database,
    value,
}: TransformerArgs<TeamMembershipModel, TeamMembership>): Promise<TeamMembershipModel> => {
    const raw = value.raw;
 ...
 */
export const transformTeamRecord = ({
    action,
    database,
    value,
}: TransformerArgs<TeamModel, Team>): Promise<TeamModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
...
export const transformTeamChannelHistoryRecord = ({
    ...
    database,
    value,
}: TransformerArgs<TeamChannelHistoryModel, TeamChannelHistory>): Promise<TeamChannelHistoryModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
 ...
 */
export const transformTeamSearchHistoryRecord = ({
    action,
    database,
    value,
}: TransformerArgs<TeamSearchHistoryModel, TeamSearchHistory>): Promise<TeamSearchHistoryModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
 ...
 */
export const transformMyTeamRecord = ({
    action,
    database,
    value,
}: TransformerArgs<MyTeamModel, MyTeam>): Promise<MyTeamModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
