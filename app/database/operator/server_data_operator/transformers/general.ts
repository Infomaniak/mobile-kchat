 ...
 */
export const transformCustomEmojiRecord = ({
    action,
    database,
    value,
}: TransformerArgs<CustomEmojiModel, CustomEmoji>): Promise<CustomEmojiModel> => {
    const raw = value.raw;
 ...
 */
export const transformRoleRecord = ({
    action,
    database,
    value,
}: TransformerArgs<RoleModel, Role>): Promise<RoleModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
 ...
 */
export const transformSystemRecord = ({
    action,
    database,
    value,
}: TransformerArgs<SystemModel, IdValue>): Promise<SystemModel> => {
    const raw = value.raw;
 ...
 */
export const transformConfigRecord = ({
    action,
    database,
    value,
}: TransformerArgs<ConfigModel, IdValue>): Promise<ConfigModel> => {
    const raw = value.raw;
 ...
 */
export const transformFileRecord = ({
    action,
    database,
    value,
}: TransformerArgs<FileModel, FileInfo>): Promise<FileModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
