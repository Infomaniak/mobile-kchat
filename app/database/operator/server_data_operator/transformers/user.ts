 ...
 */
export const transformUserRecord = ({
    action,
    database,
    value,
}: TransformerArgs<UserModel, UserProfile>): Promise<UserModel> => {
    const raw = value.raw;
 ...
 */
export const transformPreferenceRecord = ({
    action,
    database,
    value,
}: TransformerArgs<PreferenceModel, PreferenceType>): Promise<PreferenceModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
