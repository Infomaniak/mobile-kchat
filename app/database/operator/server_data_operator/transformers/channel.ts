 ...
 */
export const transformChannelRecord = ({
    action,
    database,
    value,
}: TransformerArgs<ChannelModel, Channel>): Promise<ChannelModel> => {
    const raw = value.raw;
...
export const transformMyChannelSettingsRecord = ({
    ...
    database,
    value,
}: TransformerArgs<MyChannelSettingsModel, ChannelMembership>): Promise<MyChannelSettingsModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
 ...
 */
export const transformChannelInfoRecord = ({
    action,
    database,
    value,
}: TransformerArgs<ChannelInfoModel, ChannelInfo>): Promise<ChannelInfoModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
 ...
 */
export const transformMyChannelRecord = async ({
    action,
    database,
    value,
}: TransformerArgs<MyChannelModel, ChannelMembership>): Promise<MyChannelModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
 ...
 */
export const transformChannelMembershipRecord = ({
    action,
    database,
    value,
}: TransformerArgs<ChannelMembershipModel, ChannelMembership>): Promise<ChannelMembershipModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
 ...
 */
export const transformChannelBookmarkRecord = ({
    action,
    database,
    value,
}: TransformerArgs<ChannelBookmarkModel, ChannelBookmark>): Promise<ChannelBookmarkModel> => {
    const raw = value.raw;
    const record = value.record;
    const isCreateAction = action === OperationType.CREATE;
    if (!isCreateAction && !record) {
