...
export interface ClientUsersMix {
    ...
    attachDevice: (deviceId: string, publicKey: string) => Promise<any>;
...
const ClientUsers = <TBase extends Constructor<ClientBase>>(superclass: TBase) => class extends superclass {
    ...
    attachDevice = async (deviceId: string, publicKey: string) => {
        return this.doFetch(
            `${this.getUsersRoute()}/sessions/device`,
            {method: 'put', body: {device_id: deviceId, public_key: publicKey}},
        );
    };
